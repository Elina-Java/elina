...
public class DebugContiguousScheduler<R> implements SchedulingDriver<R> {

	@Override
	public IFuture<R> schedule(SOMDTask<R> task, Object[][] partitions,
			int[][] combs, Reduction<R> red, R[] results, int nWorkers) {
		
		PartitionedTimer.endExpansion();
		PartitionedTimer.startContraction();
		...
		PartitionedTimer.endContraction();
		PartitionedTimer.startExecution();
		...
		if (red == null) {
			...
			return new DebugDistRedFuture<R>(completed,(R[]) results, red);
	}
	...
	private class Executor extends Task<R>{
		...
		public R call() throws Exception {
			...
			int index = (remainder-etid>=0 ? (tasksPerThread+1)*etid : (tasksPerThread+1)*remainder+tasksPerThread*(etid-remainder));
			int end = index + tasksPerThread + (remainder-etid>0 ? 1: 0);
			
			int nTasks = end-index;
			...
			PartitionedTimer.setThreadTasks(etid+1,nTasks);
			return null;
		}
	}
}
