...
import javaGrande.mpij.jgfutil.JGFSection1;
import mpi.MPI;
...
/* Carryout the pingpong operation on process 0 */
          for (int k=0; k<size; k++){
            MPI.COMM_WORLD.Ssend(arr_obj,0,arr_obj.length,MPI.OBJECT,1,1);
            MPI.COMM_WORLD.Recv(arr_obj,0,arr_obj.length,MPI.OBJECT,1,2);
          }
...
/* Carryout the pingpong operation on process 1 */
          for (int k=0; k<size; k++){
            MPI.COMM_WORLD.Recv(arr_obj,0,arr_obj.length,MPI.OBJECT,0,1);
            MPI.COMM_WORLD.Ssend(arr_obj,0,arr_obj.length,MPI.OBJECT,0,2);
          }
